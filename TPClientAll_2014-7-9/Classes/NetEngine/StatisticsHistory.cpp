...
bool StatisticsHistory::AddObject(TrackedObjectData tod)
{
    ...
    if (objectExists)
        return false;
    TrackedObject *to = VENet::OP_NEW<TrackedObject>(_FILE_AND_LINE_);
...
void StatisticsHistory::RemoveObjectAtIndex(unsigned int index)
{
    ...
    VENet::OP_DELETE(to, _FILE_AND_LINE_);
}
void StatisticsHistory::Clear(void)
{
    for (unsigned int idx=0; idx < objects.Size(); idx++)
    {
        VENet::OP_DELETE(objects[idx], _FILE_AND_LINE_);
    }
...
bool StatisticsHistory::AddValueByObjectID(uint64_t objectId, VEString key, SHValueType val, Time curTime, bool combineEqualTimes)
{
    unsigned int idx = GetObjectIndex(objectId);
    if (idx == (unsigned int) -1)
        return false;
...
void StatisticsHistory::AddValueByIndex(unsigned int index, VEString key, SHValueType val, Time curTime, bool combineEqualTimes)
{
    ...
    if (hi.IsInvalid())
    {
        queue = VENet::OP_NEW<TimeAndValueQueue>(_FILE_AND_LINE_);
        queue->key=key;
...
StatisticsHistory::SHErrorCode StatisticsHistory::GetHistoryForKey(uint64_t objectId, VEString key, StatisticsHistory::TimeAndValueQueue **values, Time curTime) const
{
    if (values == 0)
...
bool StatisticsHistory::GetHistorySorted(uint64_t objectId, SHSortOperation sortType, DataStructures::List<StatisticsHistory::TimeAndValueQueue *> &values) const
{
    ...
    DataStructures::List<VEString> keyList;
...
void StatisticsHistory::MergeAllObjectsOnKey(VEString key, TimeAndValueQueue *tavqOutput, SHDataCategory dataCategory) const
{
    tavqOutput->Clear();
...
void StatisticsHistory::GetUniqueKeyList(DataStructures::List<VEString> &keys)
{
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetRecentSumOfSquares(void) const
{
...
SHValueType StatisticsHistory::TimeAndValueQueue::GetLongTermLowest(void) const
{
    return longTermLowest;
}
SHValueType StatisticsHistory::TimeAndValueQueue::GetLongTermHighest(void) const
{
...
StatisticsHistory::TrackedObject::~TrackedObject()
{
    ...
    for (unsigned int idx=0; idx < itemList.Size(); idx++)
        VENet::OP_DELETE(itemList[idx], _FILE_AND_LINE_);
}
...
void StatisticsHistoryPlugin::Update(void)
{
    ...
    DataStructures::List<VENetStatistics> stats;
    vePeerInterface->GetStatisticsList(addresses, guids, stats);
    ...
    for (unsigned int idx = 0; idx < guids.Size(); idx++)
    {
        ...
        if (objectIndex!=(unsigned int)-1)
        {
            ...
            statistics.AddValueByIndex(objectIndex,
                                       "RN_lastPing",
                                       (SHValueType) vePeerInterface->GetLastPing(guids[idx]),
                                       curTime, false);
...
void StatisticsHistoryPlugin::OnClosedConnection(const SystemAddress &systemAddress, VENetGUID VENetGuid, PI2_LostConnectionReason lostConnectionReason )
{
    ...
    {
        statistics.RemoveObject(VENetGuid.g, 0);
    }
}
void StatisticsHistoryPlugin::OnNewConnection(const SystemAddress &systemAddress, VENetGUID VENetGuid, bool isIncoming)
{
    (void) systemAddress;
    (void) isIncoming;
    ...
    {
        statistics.AddObject(StatisticsHistory::TrackedObjectData(VENetGuid.g, newConnectionsObjectType, 0));
    }
}
// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
