...
void StringCompressor::AddReference(void)
{
    if (++referenceCount==1)
    {
        instance = VENet::OP_NEW<StringCompressor>( _FILE_AND_LINE_ );
    }
}
...
StringCompressor::StringCompressor()
{
    ...
    HuffmanEncodingTree *huffmanEncodingTree = VENet::OP_NEW<HuffmanEncodingTree>( _FILE_AND_LINE_ );
...
void StringCompressor::GenerateTreeFromStrings( unsigned char *input, unsigned inputLength, uint8_t languageId )
{
    ...
    if (huffmanEncodingTrees.Has(languageId))
    {
        ...
        VENet::OP_DELETE(huffmanEncodingTree, _FILE_AND_LINE_);
    }
    ...
    huffmanEncodingTree = VENet::OP_NEW<HuffmanEncodingTree>( _FILE_AND_LINE_ );
...
StringCompressor::~StringCompressor()
{
    for (unsigned i=0; i < huffmanEncodingTrees.Size(); i++)
        VENet::OP_DELETE(huffmanEncodingTrees[i], _FILE_AND_LINE_);
}

void StringCompressor::EncodeString( const char *input, int maxCharsToWrite, VENet::BitStream *output, uint8_t languageId )
{
    HuffmanEncodingTree *huffmanEncodingTree;
    ...
    VENet::BitStream encodedBitStream;
...
bool StringCompressor::DecodeString( char *output, int maxCharsToWrite, VENet::BitStream *input, uint8_t languageId )
{
    HuffmanEncodingTree *huffmanEncodingTree;
    if (huffmanEncodingTrees.Has(languageId)==false)
        return false;
...
void StringCompressor::EncodeString( const CString &input, int maxCharsToWrite, VENet::BitStream *output )
{
    LPTSTR p = input;
    EncodeString(p, maxCharsToWrite*sizeof(TCHAR), output, languageID);
}
bool StringCompressor::DecodeString( CString &output, int maxCharsToWrite, VENet::BitStream *input, uint8_t languageId )
{
...
void StringCompressor::EncodeString( const std::string &input, int maxCharsToWrite, VENet::BitStream *output, uint8_t languageId )
{
    EncodeString(input.c_str(), maxCharsToWrite, output, languageId);
}
bool StringCompressor::DecodeString( std::string *output, int maxCharsToWrite, VENet::BitStream *input, uint8_t languageId )
{
    if (maxCharsToWrite <= 0)
    {
        output->clear();
...

    {
        ...
        out=DecodeString(destinationBlock, maxCharsToWrite, input, languageId);
        *output=destinationBlock;
        veFree_Ex(destinationBlock, _FILE_AND_LINE_ );
    }

    return out;
}
#endif
void StringCompressor::EncodeString( const VEString *input, int maxCharsToWrite, VENet::BitStream *output, uint8_t languageId )
{
    EncodeString(input->C_String(), maxCharsToWrite, output, languageId);
}
bool StringCompressor::DecodeString( VEString *output, int maxCharsToWrite, VENet::BitStream *input, uint8_t languageId )
{
    if (maxCharsToWrite <= 0)
    {
        output->Clear();
