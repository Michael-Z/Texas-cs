...
FullyConnectedMesh2::FullyConnectedMesh2()
{
    ...
    hostVENetGuid=UNASSIGNED_VENET_GUID;
}
FullyConnectedMesh2::~FullyConnectedMesh2()
{
    Clear();
}
VENetGUID FullyConnectedMesh2::GetConnectedHost(void) const
{
    if (ourFCMGuid==0)
        return UNASSIGNED_VENET_GUID;
    return hostVENetGuid;
}
SystemAddress FullyConnectedMesh2::GetConnectedHostAddr(void) const
{
    if (ourFCMGuid==0)
        return UNASSIGNED_SYSTEM_ADDRESS;
    return vePeerInterface->GetSystemAddressFromGuid(hostVENetGuid);
}
VENetGUID FullyConnectedMesh2::GetHostSystem(void) const
{
    if (ourFCMGuid==0)
        return vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);

    return hostVENetGuid;
}
bool FullyConnectedMesh2::IsHostSystem(void) const
{
    return GetHostSystem()==vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
}
void FullyConnectedMesh2::GetHostOrder(DataStructures::List<VENetGUID> &hostList)
{
    hostList.Clear(true, _FILE_AND_LINE_);

    if (ourFCMGuid==0 || fcm2ParticipantList.Size()==0)
    {
        hostList.Push(vePeerInterface->GetMyGUID(), _FILE_AND_LINE_);
        return;
    }

    FCM2Participant fcm2;
    fcm2.fcm2Guid=ourFCMGuid;
    fcm2.VENetGuid=vePeerInterface->GetMyGUID();
    ...
    {
        hostList.Push(olist[i].VENetGuid, _FILE_AND_LINE_);
    }
}
bool FullyConnectedMesh2::IsConnectedHost(void) const
{
    return GetConnectedHost()==vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
}
...
{
    hostVENetGuid=UNASSIGNED_VENET_GUID;
    startupTime=VENet::GetTimeUS();
    totalConnectionCount=0;
    ourFCMGuid=0;
    for (unsigned int i=0; i < fcm2ParticipantList.Size(); i++)
        SendFCMGuidRequest(fcm2ParticipantList[i].VENetGuid);
}
bool FullyConnectedMesh2::AddParticipantInternal( VENetGUID VENetGuid, FCM2Guid theirFCMGuid )
{
    for (unsigned int i=0; i < fcm2ParticipantList.Size(); i++)
    {
        if (fcm2ParticipantList[i].VENetGuid==VENetGuid)
        {
            if (theirFCMGuid!=0)
    ...
    participant.VENetGuid=VENetGuid;
    participant.fcm2Guid=theirFCMGuid;
    fcm2ParticipantList.Push(participant,_FILE_AND_LINE_);

    SendFCMGuidRequest(VENetGuid);

    return true;
}
void FullyConnectedMesh2::AddParticipant( VENetGUID VENetGuid )
{
    if (vePeerInterface->GetConnectionState(vePeerInterface->GetSystemAddressFromGuid(VENetGuid))!=IS_CONNECTED)
    {
#ifdef DEBUG_FCM2
    ...
    AddParticipantInternal(VENetGuid,0);
}
void FullyConnectedMesh2::GetParticipantList(DataStructures::List<VENetGUID> &participantList)
{
    participantList.Clear(true, _FILE_AND_LINE_);
    unsigned int i;
    for (i=0; i < fcm2ParticipantList.Size(); i++)
        participantList.Push(fcm2ParticipantList[i].VENetGuid, _FILE_AND_LINE_);
}
bool FullyConnectedMesh2::HasParticipant(VENetGUID participantGuid)
{
    unsigned int i;
    for (i=0; i < fcm2ParticipantList.Size(); i++)
    {
        if (fcm2ParticipantList[i].VENetGuid==participantGuid)
            return true;
    }
    return false;
}
PluginReceiveResult FullyConnectedMesh2::OnReceive(Packet *packet)
{
    ...
    {
        VENet::VENetGUID g;
        VENet::BitStream b(packet->data, packet->length, false);
        b.IgnoreBits(8);
        b.Read(g);
        UpdateVerifiedJoinInProgressMember(g, UNASSIGNED_VENET_GUID, JIPS_FAILED);
        return RR_CONTINUE_PROCESSING;
    }
    case ID_NAT_PUNCHTHROUGH_FAILED:
        UpdateVerifiedJoinInProgressMember(packet->guid, UNASSIGNED_VENET_GUID, JIPS_FAILED);
        return RR_CONTINUE_PROCESSING;
    }
...
void FullyConnectedMesh2::OnClosedConnection(const SystemAddress &systemAddress, VENetGUID veNetGUID, PI2_LostConnectionReason lostConnectionReason )
{
    ...
    {
        if (joinsInProgress[idx]->requester==veNetGUID)
        {
            ...
            VENet::OP_DELETE(joinsInProgress[idx], _FILE_AND_LINE_);
    ...
    UpdateVerifiedJoinInProgressMember(veNetGUID, UNASSIGNED_VENET_GUID, JIPS_FAILED);

    for (idx=0; idx < fcm2ParticipantList.Size(); idx++)
    {
        if (fcm2ParticipantList[idx].VENetGuid==veNetGUID)
        {
            fcm2ParticipantList[idx]=fcm2ParticipantList[fcm2ParticipantList.Size()-1];
#ifdef DEBUG_FCM2
            printf("Popping participant %s\n", fcm2ParticipantList[fcm2ParticipantList.Size()-1].VENetGuid.ToString());
#endif

            fcm2ParticipantList.Pop();
            if (veNetGUID==hostVENetGuid && ourFCMGuid!=0)
            {
                if (fcm2ParticipantList.Size()==0)
                {
                    hostVENetGuid=vePeerInterface->GetMyGUID();
                    hostFCM2Guid=ourFCMGuid;
                }
                else
                {
                    CalculateHost(&hostVENetGuid, &hostFCM2Guid);
                }
                PushNewHost(hostVENetGuid, veNetGUID);
            }
            return;
        }
    }
...
VENet::TimeUS FullyConnectedMesh2::GetElapsedRuntime(void)
{
    VENet::TimeUS curTime=VENet::GetTimeUS();
    if (curTime>startupTime)
        return curTime-startupTime;
    else
        return 0;
}
void FullyConnectedMesh2::OnNewConnection(const SystemAddress &systemAddress, VENetGUID veNetGUID, bool isIncoming)
{
    (void) isIncoming;
    (void) veNetGUID;
    (void) systemAddress;

    UpdateVerifiedJoinInProgressMember(systemAddress, veNetGUID, JIPS_CONNECTED);

    if (autoParticipateConnections)
        AddParticipant(veNetGUID);
}
void FullyConnectedMesh2::OnFailedConnectionAttempt(Packet *packet, PI2_FailedConnectionAttemptReason failedConnectionAttemptReason)
{
    ...
    {
        UpdateVerifiedJoinInProgressMember(packet->systemAddress, UNASSIGNED_VENET_GUID, JIPS_FAILED);
    }
}
void FullyConnectedMesh2::Clear(void)
{
    ...
    {
        VENet::OP_DELETE(joinsInProgress[i], _FILE_AND_LINE_);
    }
    joinsInProgress.Clear(true, _FILE_AND_LINE_);

    totalConnectionCount=0;
    ourFCMGuid=0;
    lastPushedHost=UNASSIGNED_VENET_GUID;
}
void FullyConnectedMesh2::PushNewHost(const VENetGUID &guid, VENetGUID oldHost)
{
    Packet *p = AllocatePacketUnified(sizeof(MessageID)+sizeof(oldHost));
    VENet::BitStream bs(p->data,p->length,false);
    bs.SetWriteOffset(0);
    bs.Write((MessageID)ID_FCM2_NEW_HOST);
    bs.Write(oldHost);
    p->systemAddress=vePeerInterface->GetSystemAddressFromGuid(guid);
    p->systemAddress.systemIndex=(SystemIndex)-1;
    p->guid=guid;
    p->wasGeneratedLocally=true;
    vePeerInterface->PushBackPacket(p, true);
...
void FullyConnectedMesh2::SendFCMGuidRequest(VENetGUID veNetGuid)
{
    if (veNetGuid==vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS))
        return;
    ...
    vePeerInterface->Send(&bsOut,HIGH_PRIORITY,RELIABLE_ORDERED,0,veNetGuid,false);
}
void FullyConnectedMesh2::SendOurFCMGuid(SystemAddress addr)
{
    VENet::BitStream bsOut;
    bsOut.Write((MessageID)ID_FCM2_INFORM_FCMGUID);
    VEAssert(ourFCMGuid!=0);
    bsOut.Write(ourFCMGuid);
    bsOut.Write(totalConnectionCount);
    vePeerInterface->Send(&bsOut,HIGH_PRIORITY,RELIABLE_ORDERED,0,addr,false);
}
...
void FullyConnectedMesh2::AssignOurFCMGuid(void)
{
    ...
    randomNumber ^= (unsigned int) (VENet::GetTimeUS() & 0xFFFFFFFF);
    randomNumber ^= (unsigned int) (vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS).g & 0xFFFFFFFF);
    ourFCMGuid |= randomNumber;
    uint64_t reponse64 = totalConnectionCount;
    ourFCMGuid |= reponse64<<32;
}
void FullyConnectedMesh2::CalculateHost(VENetGUID *VENetGuid, FCM2Guid *fcm2Guid)
{
    // Can't calculate host without knowing our own
    VEAssert(ourFCMGuid!=0);

    // Can't calculate host without being connected to anyone else
    VEAssert(fcm2ParticipantList.Size()>0);
    ...
    VENetGUID associatedVENetGuid=vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
    ...
    for (idx=0; idx < fcm2ParticipantList.Size(); idx++)
    {
        if (fcm2ParticipantList[idx].fcm2Guid!=0 && fcm2ParticipantList[idx].fcm2Guid<lowestFCMGuid)
        {
            ...
            associatedVENetGuid=fcm2ParticipantList[idx].VENetGuid;
        }
    }

    *VENetGuid=associatedVENetGuid;
...
void FullyConnectedMesh2::OnRequestFCMGuid(Packet *packet)
{
    ...
    VENet::TimeUS senderElapsedRuntime=0;
    ...
    if (ourFCMGuid==0)
    {
        if (hasRemoteFCMGuid==false)
        {
            ...
            VENet::TimeUS ourElapsedRuntime = GetElapsedRuntime();
        ...
        else
        {
            ...
            unsigned int idx;
            for (idx=0; idx < fcm2ParticipantList.Size(); idx++)
                SendOurFCMGuid(vePeerInterface->GetSystemAddressFromGuid(fcm2ParticipantList[idx].VENetGuid));
        }
    }
    else
    {
...
void FullyConnectedMesh2::OnInformFCMGuid(Packet *packet)
{
    ...
    if (AddParticipantInternal(packet->guid,theirFCMGuid))
    {
        ...
        for (idx=0; idx < fcm2ParticipantList.Size(); idx++)
        {
            if (packet->guid!=fcm2ParticipantList[idx].VENetGuid)
                vePeerInterface->Send(&bsOut,HIGH_PRIORITY,RELIABLE_ORDERED,0,fcm2ParticipantList[idx].VENetGuid,false);
        }
    }
...
void FullyConnectedMesh2::CalculateAndPushHost(void)
{
    ...
    if (ParticipantListComplete())
    {
        ...
        {
            hostVENetGuid=newHostGuid;
            hostFCM2Guid=newFcmGuid;
            PushNewHost(hostVENetGuid, lastPushedHost);
        }
    }
}
...
void FullyConnectedMesh2::SetConnectOnNewRemoteConnection(bool attemptConnection, VENet::VEString pw)
{
...
void FullyConnectedMesh2::ConnectToRemoteNewIncomingConnections(Packet *packet)
{
    ...
    for (unsigned int i=0; i < count; i++)
    {
        ...
        vePeerInterface->Connect(str,remoteAddress.GetPort(),connectionPassword.C_String(),(int) connectionPassword.GetLength());
    }
}
...
void FullyConnectedMesh2::StartVerifiedJoin(VENetGUID client)
{
    VEAssert(HasParticipant(client)==false);
    VEAssert(client!=vePeerInterface->GetMyGUID());
    ...
    for (i=0; i < fcm2ParticipantList.Size(); i++)
    {
        bsOut.Write(fcm2ParticipantList[i].VENetGuid);
        bsOut.Write(vePeerInterface->GetSystemAddressFromGuid(fcm2ParticipantList[i].VENetGuid));
    }
...
void FullyConnectedMesh2::RespondOnVerifiedJoinCapable(Packet *packet, bool accept, BitStream *additionalData)
{
    ...
    if (participatingMembersNotOnClient.Size()>0)
    {
        ...
        for (i=0; i < participatingMembersNotOnClient.Size(); i++)
        {
            ...
            bsOut.Write(vePeerInterface->GetSystemAddressFromGuid(participatingMembersNotOnClient[i]));
        }
        SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, packet->guid, false);
        return;
    }

    VEAssert(participatingMembersOnClientFailed.Size()==0);
    VEAssert(participatingMembersNotOnClient.Size()==0);
    ...
    if (accept)
    {
        ...
        for (unsigned int i=0; i < fcm2ParticipantList.Size(); i++)
            SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0, fcm2ParticipantList[i].VENetGuid, false);
...
void FullyConnectedMesh2::GetVerifiedJoinRequiredProcessingList(VENetGUID host, DataStructures::List<SystemAddress> &addresses, DataStructures::List<VENetGUID> &guids)
{
...
void FullyConnectedMesh2::GetVerifiedJoinAcceptedAdditionalData(Packet *packet, bool *thisSystemAccepted, DataStructures::List<VENetGUID> &systemsAccepted, BitStream *additionalData)
{
    systemsAccepted.Clear(true, _FILE_AND_LINE_);

    VENet::BitStream bsIn(packet->data, packet->length, false);
    bsIn.IgnoreBytes(sizeof(MessageID));
    VENetGUID systemToAddGuid;
    bsIn.Read(systemToAddGuid);
    *thisSystemAccepted = systemToAddGuid == vePeerInterface->GetMyGUID();
    unsigned short listSize;
    bsIn.Read(listSize);
    bsIn.IgnoreBytes(listSize*VENetGUID::size());
    bsIn.Read(listSize);
    if (systemToAddGuid==vePeerInterface->GetMyGUID())
    {
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinStart(Packet *packet)
{
    ...
    if (curIndex!=(unsigned int) -1)
    {
        ...
        for (unsigned short i=0; i < listSize; i++)
        {
            ...
            unsigned int j;
            if (vjipm.guid!=UNASSIGNED_VENET_GUID)
    ...
    VerifiedJoinInProgress *vjip = VENet::OP_NEW<VerifiedJoinInProgress>(_FILE_AND_LINE_);
...
PluginReceiveResult FullyConnectedMesh2::OnVerifiedJoinCapable(Packet *packet)
{
    ...
    VEAssert(HasParticipant(packet->guid)==false);
...
void FullyConnectedMesh2::OnVerifiedJoinFailed(VENetGUID hostGuid)
{
    ...
    for (unsigned int j=0; j < vjip->members.Size(); j++)
    {
        ...
        {
            vePeerInterface->CloseConnection(vjip->members[j].guid, true);
        }
    }

    VENet::OP_DELETE(joinsInProgress[curIndex], _FILE_AND_LINE_);
...
void FullyConnectedMesh2::OnVerifiedJoinAccepted(Packet *packet)
{
    ...
    if (systemToAddGuid==vePeerInterface->GetMyGUID())
    {
        ...
        for (unsigned short i=0; i < listSize; i++)
        {
            VENetGUID guid;
            bsIn.Read(guid);
            vePeerInterface->CloseConnection(guid, true);
        }
    ...
    {
        ConnectionState cs = vePeerInterface->GetConnectionState(systemToAddGuid);
        VEAssert(cs==IS_CONNECTED);
        if (cs==IS_CONNECTED)
...
unsigned int FullyConnectedMesh2::GetJoinsInProgressIndex(VENetGUID requester) const
{
    for (unsigned int i=0; i < joinsInProgress.Size(); i++)
    {
...
void FullyConnectedMesh2::UpdateVerifiedJoinInProgressMember(const AddressOrGUID systemIdentifier, VENetGUID guidToAssign, FullyConnectedMesh2::JoinInProgressState newState)
{
    ...
    for (unsigned int i=0; i < joinsInProgress.Size(); i++)
    {
        ...
        if (j!=(unsigned int)-1)
        {
            if (vjip->members[j].guid==UNASSIGNED_VENET_GUID && guidToAssign!=UNASSIGNED_VENET_GUID)
...
void FullyConnectedMesh2::ReadVerifiedJoinInProgressMember(VENet::BitStream *bsIn, VerifiedJoinInProgressMember *vjipm)
{
    bsIn->Read(vjipm->guid);
    bsIn->Read(vjipm->systemAddress);
    ConnectionState cs = vePeerInterface->GetConnectionState(vjipm->guid);
    if (cs==IS_CONNECTED)
...
unsigned int FullyConnectedMesh2::GetVerifiedJoinInProgressMemberIndex(const AddressOrGUID systemIdentifier, VerifiedJoinInProgress *vjip)
{
    for (unsigned int j=0; j < vjip->members.Size(); j++)
    {
        if ((systemIdentifier.veNetGuid!=UNASSIGNED_VENET_GUID && vjip->members[j].guid==systemIdentifier.veNetGuid) ||
        ...
        DataStructures::List<VENetGUID> &clientMembersNotParticipatingFailed)
{
    ...
    for (unsigned int i=0; i < fcm2ParticipantList.Size(); i++)
    {
        unsigned int j = GetVerifiedJoinInProgressMemberIndex(fcm2ParticipantList[i].VENetGuid, vjip);
        if (j==(unsigned int)-1)
        {
            participatingMembersNotOnClient.Push(fcm2ParticipantList[i].VENetGuid, _FILE_AND_LINE_);
        }
        else
        {
            if (vjip->members[j].joinInProgressState==JIPS_FAILED)
                participatingMembersOnClientFailed.Push(fcm2ParticipantList[i].VENetGuid, _FILE_AND_LINE_);
            else
                participatingMembersOnClientSucceeded.Push(fcm2ParticipantList[i].VENetGuid, _FILE_AND_LINE_);
            vjip->members[j].workingFlag=true;
        }
    }
