...
{
    (void) index;
    input->cells.Insert(VENet::OP_NEW<Table::Cell>(_FILE_AND_LINE_), _FILE_AND_LINE_ );
}
void FreeRow(Table::Row* input, int index)
{
    ...
    {
        VENet::OP_DELETE(input->cells[i], _FILE_AND_LINE_);
    }
    VENet::OP_DELETE(input, _FILE_AND_LINE_);
}
Table::Cell::Cell()
{
    isEmpty=true;
...
VENet::VEString Table::Cell::ToString(ColumnType columnType)
{
    if (isEmpty)
        return VENet::VEString();

    if (columnType==NUMERIC)
    {
        return VENet::VEString("%f", i);
    }
    else if (columnType==STRING)
    {
        return VENet::VEString(c);
    }
    else if (columnType==BINARY)
    {
        return VENet::VEString("<Binary>");
    }
    else if (columnType==POINTER)
    {
        return VENet::VEString("%p", ptr);
    }
...
void Table::RemoveColumn(unsigned columnIndex)
{
    ...
    while (cur)
    {
        ...
        {
            VENet::OP_DELETE(cur->data[i]->cells[columnIndex], _FILE_AND_LINE_);
...
{
    Row *newRow;
    newRow = VENet::OP_NEW<Row>( _FILE_AND_LINE_ );
    if (rows.Insert(rowId, newRow)==false)
    {
        VENet::OP_DELETE(newRow, _FILE_AND_LINE_);
        return 0;
    }
    unsigned rowIndex;
    for (rowIndex=0; rowIndex < columns.Size(); rowIndex++)
        newRow->cells.Insert( VENet::OP_NEW<Table::Cell>(_FILE_AND_LINE_), _FILE_AND_LINE_ );
    return newRow;
}
Table::Row* Table::AddRow(unsigned rowId, DataStructures::List<Cell> &initialCellValues)
{
    Row *newRow = VENet::OP_NEW<Row>( _FILE_AND_LINE_ );
    unsigned rowIndex;
    for (rowIndex=0; rowIndex < columns.Size(); rowIndex++)
    {
        ...
        {
            Table::Cell *c;
            c = VENet::OP_NEW<Table::Cell>(_FILE_AND_LINE_);
...
Table::Row* Table::AddRow(unsigned rowId, DataStructures::List<Cell*> &initialCellValues, bool copyCells)
{
    ...
    for (rowIndex=0; rowIndex < columns.Size(); rowIndex++)
    {
        ...
        {
            if (copyCells==false)
                newRow->cells.Insert(VENet::OP_NEW_4<Table::Cell>( _FILE_AND_LINE_, initialCellValues[rowIndex]->i, initialCellValues[rowIndex]->c, initialCellValues[rowIndex]->ptr, columns[rowIndex].columnType), _FILE_AND_LINE_);
            else
            {
                Table::Cell *c = VENet::OP_NEW<Table::Cell>( _FILE_AND_LINE_ );
                newRow->cells.Insert(c, _FILE_AND_LINE_);
...
Table::Row* Table::AddRowColumns(unsigned rowId, Row *row, DataStructures::List<unsigned> columnIndices)
{
    ...
    for (columnIndex=0; columnIndex < columnIndices.Size(); columnIndex++)
    {
        ...
        {
            newRow->cells.Insert(VENet::OP_NEW_4<Table::Cell>( _FILE_AND_LINE_,
...
{
    VEAssert(columns[columnIndex].columnType==NUMERIC);
...
{
    VEAssert(columns[columnIndex].columnType==STRING);
...
{
    VEAssert(columns[columnIndex].columnType==BINARY);
...
void Table::SortTable(Table::SortQuery *sortQueries, unsigned numSortQueries, Table::Row** out)
{
    ...
    for (i=0; i < numSortQueries; i++)
    {
        if (sortQueries[i].columnIndex<columns.Size() && columns[sortQueries[i].columnIndex].columnType!=BINARY)
        {
            ...
            columnIndices.Insert((unsigned)-1, _FILE_AND_LINE_);
    }
...
void Table::DeleteRow(Table::Row *row)
{
    ...
    {
        VENet::OP_DELETE(row->cells[rowIndex], _FILE_AND_LINE_);
    }
    VENet::OP_DELETE(row, _FILE_AND_LINE_);
}
