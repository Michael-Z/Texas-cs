...
void PacketizedTCP::Stop(void)
{
    unsigned int i;
    for (i=0; i < messageHandlerList.Size(); i++)
        messageHandlerList[i]->OnVEPeerShutdown();
...
void PacketizedTCP::Send( const char *data, unsigned length, const SystemAddress &systemAddress, bool broadcast )
{
    ...
    if (VENet::BitStream::DoEndianSwap())
        VENet::BitStream::ReverseBytes((unsigned char*) &length,(unsigned char*) &dataLength,sizeof(dataLength));
#else
    dataLength=length;
#endif
...
#ifndef __BITSTREAM_NATIVE_END
    if (VENet::BitStream::DoEndianSwap())
        VENet::BitStream::ReverseBytes((unsigned char*) &totalLengthOfUserData,(unsigned char*) &dataLength,sizeof(dataLength));
#else
...
void PacketizedTCP::PushNotificationsToQueues(void)
{
    ...
    if (sa!=UNASSIGNED_SYSTEM_ADDRESS)
    {
        ...
        unsigned int i;
        for (i=0; i < messageHandlerList.Size(); i++)
            messageHandlerList[i]->OnNewConnection(sa, UNASSIGNED_VENET_GUID, true);
    }
    ...
    if (sa!=UNASSIGNED_SYSTEM_ADDRESS)
    {
        ...
        unsigned int i;
        for (i=0; i < messageHandlerList.Size(); i++)
            messageHandlerList[i]->OnClosedConnection(sa, UNASSIGNED_VENET_GUID, LCR_DISCONNECTION_NOTIFICATION);
    }
    ...
    while (incomingPacket)
    {
        ...
        if (incomingPacket->deleteData==true)
        {
            ...
            if (index < connections.Size())
            {
                ...
                if (VENet::BitStream::DoEndianSwap())
                    VENet::BitStream::ReverseBytesInPlace((unsigned char*) &dataLength,sizeof(dataLength));
                if (bq->GetBytesWritten()>=dataLength+sizeof(PTCPHeader))
                {
                    do
                    {
                        bq->IncrementReadOffset(sizeof(PTCPHeader));
                        outgoingPacket = VENet::OP_NEW<Packet>(_FILE_AND_LINE_);
                        outgoingPacket->length=dataLength;
                        outgoingPacket->bitSize=BYTES_TO_BITS(dataLength);
                        outgoingPacket->guid=UNASSIGNED_VENET_GUID;
                        ...
                        if (outgoingPacket->data==0)
                        {
                            ...
                            VENet::OP_DELETE(outgoingPacket,_FILE_AND_LINE_);
                            return 0;
                        }
...
void PacketizedTCP::CloseConnection( SystemAddress systemAddress )
{
    ...
    unsigned int i;
    for (i=0; i < messageHandlerList.Size(); i++)
        messageHandlerList[i]->OnClosedConnection(systemAddress, UNASSIGNED_VENET_GUID, LCR_CLOSED_BY_USER);
...
void PacketizedTCP::RemoveFromConnectionList(const SystemAddress &sa)
{
    ...
    if (connections.Has(sa))
    {
        ...
        {
            VENet::OP_DELETE(connections[index],_FILE_AND_LINE_);
...
void PacketizedTCP::AddToConnectionList(const SystemAddress &sa)
{
    if (sa==UNASSIGNED_SYSTEM_ADDRESS)
        return;
    connections.SetNew(sa, VENet::OP_NEW<DataStructures::ByteQueue>(_FILE_AND_LINE_));
}
void PacketizedTCP::ClearAllConnections(void)
{
    unsigned int i;
    for (i=0; i < connections.Size(); i++)
        VENet::OP_DELETE(connections[i],_FILE_AND_LINE_);
    connections.Clear();
}
