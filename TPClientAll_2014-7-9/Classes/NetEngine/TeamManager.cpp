...
bool TM_TeamMember::RequestTeam(TeamSelection teamSelection)
{
    if (teamSelection.joinTeamType==JOIN_NO_TEAM)
    {
        ...
        world->BroadcastToParticipants(&bsOut, UNASSIGNED_VENET_GUID);
    ...
    {
        VEAssert(teamSelection.joinTeamType==JOIN_SPECIFIC_TEAM);
...
bool TM_TeamMember::DeserializeConstruction(TeamManager *teamManager, BitStream *constructionBitstream)
{
    ...
    for (unsigned int i=0; i < teamsRequestedSize; i++)
    {
        ...
        if (hasTeamRequested)
        {
            ...
            VEAssert(rt.requested);
        }
        rt.whenRequested=VENet::GetTime();
        rt.requestIndex=world->teamRequestIndex++;
        if (
...
void TM_TeamMember::UpdateTeamsRequestedToAny(void)
{
    ...
    whenJoinAnyRequested=VENet::GetTime();
    joinAnyRequestIndex=world->teamRequestIndex++;
}
...
void TM_World::AddParticipant(VENetGUID VENetGuid)
{
    participants.Push(VENetGuid, _FILE_AND_LINE_ );
    ...
    if (GetTeamManager()->GetMyGUIDUnified()==GetHost())
    {
        ...
        teamManager->SendUnified(&bsOut, HIGH_PRIORITY, RELIABLE_ORDERED, 0,VENetGuid, false);
    }
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_World::RemoveParticipant(VENetGUID VENetGuid)
{
    unsigned int i;
    i = participants.GetIndexOf(VENetGuid);
    if (i!=(unsigned int)-1)
...
void TM_World::GetParticipantList(DataStructures::List<VENetGUID> &participantList)
{
...
void TM_World::SetHost(VENetGUID _hostGuid)
{
    if (hostGuid==_hostGuid)
        return;

    VEAssert(_hostGuid!=UNASSIGNED_VENET_GUID);
...
VENetGUID TM_World::GetHost(void) const
{
    return hostGuid;
}
...
void TM_World::OnClosedConnection(const SystemAddress &systemAddress, VENetGUID VENetGuid, PI2_LostConnectionReason lostConnectionReason )
{
    (void) lostConnectionReason;
    (void) systemAddress;

    RemoveParticipant(VENetGuid);
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TM_World::OnNewConnection(const SystemAddress &systemAddress, VENetGUID VENetGuid, bool isIncoming)
{
    (void) isIncoming;
    (void) systemAddress;
...
{
    VEAssert(GetHost()==GetTeamManager()->GetMyGUIDUnified());
    ...
    for (i=0; i < teams.Size(); i++)
    {
                ...
                (balancedTeamLimit < team->GetTeamMembersCount() && team->GetBalancingApplies()) )
        {
            ...
            else
            {
                ...
                bsOut.Write(true);
                bsOut.Write(team->GetNetworkID());
                BroadcastToParticipants(&bsOut, UNASSIGNED_VENET_GUID);
            }
...
void TM_World::BroadcastToParticipants(VENet::BitStream *bsOut, VENetGUID exclusionGuid)
{
...
void TM_World::BroadcastToParticipants(unsigned char *data, const int length, VENetGUID exclusionGuid)
{
    for (unsigned int i=0; i < participants.Size(); i++)
    {
        if (participants[i]==exclusionGuid)
        ...
        else
        {
            if (teamsWeAreLeaving.Size()==1)
            {
                ...
                if (j!=(unsigned int)-1)
                {
                    ...
                    BroadcastToParticipants(&bitStream, UNASSIGNED_VENET_GUID);
...
TM_World* TeamManager::AddWorld(WorldId worldId)
{
    ...
    TM_World *newWorld = VENet::OP_NEW<TM_World>(_FILE_AND_LINE_);
...
void TeamManager::EncodeTeamFull(VENet::BitStream *bitStream, TM_TeamMember *teamMember, TM_Team *team)
{
...
void TeamManager::EncodeTeamLocked(VENet::BitStream *bitStream, TM_TeamMember *teamMember, TM_Team *team)
{
...
void TeamManager::EncodeTeamFullOrLocked(VENet::BitStream *bitStream, TM_TeamMember *teamMember, TM_Team *team)
{
...
void TeamManager::DecomposeTeamFullOrLocked(VENet::BitStream *bsIn, TM_World **world, TM_TeamMember **teamMember, TM_Team **team,
        uint16_t &currentMembers, uint16_t &memberLimitIncludingBalancing, bool &balancingIsActive, JoinPermissions &joinPermissions)
{
    WorldId worldId;
...
void TeamManager::EncodeTeamAssigned(VENet::BitStream *bitStream, TM_TeamMember *teamMember)
{
    bitStream->Write(teamMember->world->GetWorldId());
    bitStream->Write(teamMember->GetNetworkID());
...
void TeamManager::ProcessTeamAssigned(VENet::BitStream *bsIn)
{
    TM_World *world;
    TM_TeamMember *teamMember;
    NoTeamId noTeamId;
...
void TeamManager::OnClosedConnection(const SystemAddress &systemAddress, VENetGUID VENetGuid, PI2_LostConnectionReason lostConnectionReason )
{
    for (unsigned int i=0; i < worldsList.Size(); i++)
    {
        worldsList[i]->OnClosedConnection(systemAddress, VENetGuid, lostConnectionReason);
    }
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TeamManager::OnNewConnection(const SystemAddress &systemAddress, VENetGUID VENetGuid, bool isIncoming)
{
    for (unsigned int i=0; i < worldsList.Size(); i++)
    {
        worldsList[i]->OnNewConnection(systemAddress, VENetGuid, isIncoming);
    }
}

// --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

void TeamManager::Send( const VENet::BitStream * bitStream, const AddressOrGUID systemIdentifier, bool broadcast )
{
...
void TeamManager::PushBitStream(VENet::BitStream *bitStream)
{
...
void TeamManager::OnJoinAnyTeam(Packet *packet, TM_World *world)
{
    ...
    if (teamMember)
    {
        VEAssert(world->GetHost()==world->GetTeamManager()->GetMyGUIDUnified());
    ...
    if (team)
    {
        ...
        if (world->GetHost()==world->GetTeamManager()->GetMyGUIDUnified())
        {
            if (packet->guid==GetMyGUIDUnified())
                ...
                world->BroadcastToParticipants(packet->data, packet->length, UNASSIGNED_VENET_GUID);
            world->FillRequestedSlots();
