...
void PacketLogger::OnDirectSocketSend(const char *data, const BitSize_t bitsUsed, SystemAddress remoteSystemAddress)
{
    ...
    char str[256];
    FormatLine(str, "Snd", "Raw", 0, 0, data[0], bitsUsed, VENet::GetTimeMS(), vePeerInterface->GetExternalID(remoteSystemAddress), remoteSystemAddress, (unsigned int)-1,(unsigned int)-1,(unsigned int)-1,(unsigned int)-1);
    AddToLog(str);
}
...
void PacketLogger::OnDirectSocketReceive(const char *data, const BitSize_t bitsUsed, SystemAddress remoteSystemAddress)
{
    ...
    char str[256];
    FormatLine(str, "Rcv", "Raw", 0, 0, data[0], bitsUsed, VENet::GetTime(), vePeerInterface->GetInternalID(UNASSIGNED_SYSTEM_ADDRESS), remoteSystemAddress,(unsigned int)-1,(unsigned int)-1,(unsigned int)-1,(unsigned int)-1);
    AddToLog(str);
}
void PacketLogger::OnReliabilityLayerNotification(const char *errorMessage, const BitSize_t bitsUsed, SystemAddress remoteSystemAddress, bool isError)
{
    ...
    if (isError)
        ...
        type=(char*) "RcvWrn";
    FormatLine(str, type, errorMessage, 0, 0, "", bitsUsed, VENet::GetTime(), vePeerInterface->GetInternalID(UNASSIGNED_SYSTEM_ADDRESS), remoteSystemAddress,(unsigned int)-1,(unsigned int)-1,(unsigned int)-1,(unsigned int)-1);
    AddToLog(str);
    VEAssert(isError==false);
}
void PacketLogger::OnAck(unsigned int messageNumber, SystemAddress remoteSystemAddress, VENet::TimeMS time)
{
    char str[256];
    char str1[64], str2[62];
    SystemAddress localSystemAddress = vePeerInterface->GetExternalID(remoteSystemAddress);
...
void PacketLogger::OnInternalPacket(InternalPacket *internalPacket, unsigned frameNumber, SystemAddress remoteSystemAddress, VENet::TimeMS time, int isSend)
{
    char str[256];
    ...
    {
        FormatLine(str, sendType, "Tms", reliableMessageNumber, frameNumber, internalPacket->data[1+sizeof(VENet::Time)], internalPacket->dataBitLength, (unsigned long long)time, localSystemAddress, remoteSystemAddress, internalPacket->splitPacketId, internalPacket->splitPacketIndex, internalPacket->splitPacketCount, internalPacket->orderingIndex);
    }
    else
    {
...
{
    VENET_DEBUG_PRINTF("%s\n", str);
}
void PacketLogger::WriteMiscellaneous(const char *type, const char *msg)
{
    char str[1024];
    char str1[64];
    SystemAddress localSystemAddress = vePeerInterface->GetInternalID();
    localSystemAddress.ToString(true, str1);
    VENet::TimeMS time = VENet::GetTimeMS();
    char localtime[128];
    GetLocalTime(localtime);
