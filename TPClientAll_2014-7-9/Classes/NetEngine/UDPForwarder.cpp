...
namespace VENet
{
VE_THREAD_DECLARATION(UpdateUDPForwarderGlobal);
}

UDPForwarder::ForwardEntry::ForwardEntry()
{
    socket=INVALID_SOCKET;
    timeLastDatagramForwarded=VENet::GetTimeMS();
...
void UDPForwarder::Startup(void)
{
    ...
    errorCode = VENet::RakThread::Create(UpdateUDPForwarderGlobal, this);
...
UDPForwarderResult UDPForwarder::StartForwarding(SystemAddress source, SystemAddress destination, VENet::TimeMS timeoutOnNoDataMS, const char *forceHostAddress, unsigned short socketFamily,
...
void UDPForwarder::RecvFrom(VENet::TimeMS curTime, ForwardEntry *forwardEntry)
{
    ...
    if (receivedDataLen<0)
    {
        ...
        if (dwIOError!=WSAECONNRESET && dwIOError!=WSAEINTR && dwIOError!=WSAETIMEDOUT && dwIOError!=WSAEWOULDBLOCK)
        {
            ...
            VENET_DEBUG_PRINTF( "recvfrom failed:Error code - %d\n%s", dwIOError, messageBuffer );
...
void UDPForwarder::UpdateUDPForwarder(void)
{
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (sfos.result==UDPFORWARDER_RESULT_COUNT)
            {
                ...
                ForwardEntry *fe = VENet::OP_NEW<UDPForwarder::ForwardEntry>(_FILE_AND_LINE_);
                ...
                if (ret==-1)
                {
                    VENet::OP_DELETE(fe,_FILE_AND_LINE_);
    ...
    {
        if (curTime > forwardListNotUpdated[i]->timeLastDatagramForwarded &&
                curTime > forwardListNotUpdated[i]->timeLastDatagramForwarded+forwardListNotUpdated[i]->timeoutOnNoDataMS)
        {
            VENet::OP_DELETE(forwardListNotUpdated[i],_FILE_AND_LINE_);
