...
void VariableDeltaSerializer::OnMessageReceipt(VENetGUID guid, uint32_t receiptId, bool messageArrived)
{
	if (messageArrived)
...
void VariableDeltaSerializer::BeginUnreliableAckedSerialize(SerializationContext *context, VENetGUID _guid, BitStream *_bitStream, uint32_t _sendReceipt)
{
	VEAssert(_guid!=UNASSIGNED_VENET_GUID);
...
void VariableDeltaSerializer::BeginUniqueSerialize(SerializationContext *context, VENetGUID _guid, BitStream *_bitStream)
{
	VEAssert(_guid!=UNASSIGNED_VENET_GUID);
	context->anyVariablesWritten=false;
	context->guid=_guid;
...
{
	context->anyVariablesWritten=false;
	context->guid=UNASSIGNED_VENET_GUID;
	context->bitStream=_bitStream;
	context->serializationMode=RELIABLE;
	if (context->variableHistoryIdentical==0)
		context->variableHistoryIdentical=StartVariableHistoryWrite(UNASSIGNED_VENET_GUID);
...
void VariableDeltaSerializer::AddRemoteSystemVariableHistory(VENetGUID guid)
{
	(void) guid;
}

void VariableDeltaSerializer::RemoveRemoteSystemVariableHistory(VENetGUID guid)
{
...
int VENet::VariableDeltaSerializer::UpdatedVariablesListPtrComp( const uint32_t &key, ChangedVariablesList* const &data )
{
...
void VariableDeltaSerializer::FreeVarsAssociatedWithReceipt(VENetGUID guid, uint32_t receiptId)
{
	unsigned int idx, idx2;
	idx = GetVarsWrittenPerRemoteSystemListIndex(guid);
	if (idx==(unsigned int)-1)
...
void VariableDeltaSerializer::DirtyAndFreeVarsAssociatedWithReceipt(VENetGUID guid, uint32_t receiptId)
{
	unsigned int idx, idx2;
	idx = GetVarsWrittenPerRemoteSystemListIndex(guid);
	if (idx==(unsigned int)-1)
...
unsigned int VariableDeltaSerializer::GetVarsWrittenPerRemoteSystemListIndex(VENetGUID guid)
{
	unsigned int idx;
...
VariableDeltaSerializer::RemoteSystemVariableHistory* VariableDeltaSerializer::GetRemoteSystemVariableHistory(VENetGUID guid)
{
...
VariableDeltaSerializer::RemoteSystemVariableHistory *VariableDeltaSerializer::StartVariableHistoryWrite(VENetGUID guid)
{
...
unsigned int VariableDeltaSerializer::GetRemoteSystemHistoryListIndex(VENetGUID guid)
{
	unsigned int idx;
	for (idx=0; idx < remoteSystemVariableHistoryList.Size(); idx++)
	{
