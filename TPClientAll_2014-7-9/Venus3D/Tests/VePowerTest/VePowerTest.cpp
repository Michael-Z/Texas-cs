...
void PrintIntList(VeList<VeInt32>& kList)
{
	for(VeList<VeInt32>::iterator it = kList.Begin(); it != kList.End(); ++it)
	{
		...
		if(it.IsLast())
		{
			VE_LOG_IString("ListTest", "Iter Last");
		}
	}
	...
	for(VeList<VeInt32>::iterator it = kList.Last(); it != kList.End(); --it)
	{
		...
		if(it.IsFirst())
		{
			VE_LOG_IString("ListTest", "Iter First");
		}
	}
}
...
void TestTreeMap()
{
	...
	{
		VE_LOG_I("MapTest", "%d: %d", iter->m_tFirst, iter->m_tSecond);
	}
}
...
void TestString()
{
	VeStringA kVecStr = "0.5f,0.8f";
	VeVector2D kVec2D;
	kVecStr.ToVector2D(kVec2D);
	VeDebugOutputString(VeStringA::From(kVec2D));

	VeVector2D kVec2DW;
	VeStringW kVecStrW = L"0.5f,0.8f";
	kVecStrW.ToVector2D(kVec2DW);
	VeDebugOutputString(VeStringA::From(kVec2DW));
...
void ReadCallback(const VeMemoryIStreamPtr& spData)
{
    if(spData)
    {
        VeDebugOutputString((const VeChar8*)spData->GetData());
        spData->Finish();
    }
}

void WriteCallback(VE_RESULT eRes)
{
	if(VE_SUCCEEDED(eRes))
	{
		VeDebugOutputString("Write async succeed");
	}
	else
	{
		VeDebugOutputString("Write async failed");
	}
}
...
#	define ZIP_FILE "Test.zip"
	...
	if(kDir.Change("Test"))
	{
		VeDebugOutputString("Change To Test Dir");
