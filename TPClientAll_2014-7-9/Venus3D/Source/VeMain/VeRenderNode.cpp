	...
	: VeRenderTick(VeRenderTick::PS_SHADER_RESOURCE), m_u32Slot(u32Slot)
{
	...
	VE_ASSERT(m_spShaderResourceView);
}
//--------------------------------------------------------------------------
...
VeIAVertexBuffer::VeIAVertexBuffer(const VeChar8* pcName,
	VeUInt32 u32Stride, VeUInt32 u32Slot)
	: VeRenderTick(VeRenderTick::IA_VERTEX_BUFFER)
	, m_u32Stride(u32Stride), m_u32Slot(u32Slot)
{
	g_pRenderer->GetRenderObject(pcName, m_spBuffer);
	VE_ASSERT(m_spBuffer);
}
...
//--------------------------------------------------------------------------
VeDrawCall::VeDrawCall(VeUInt32 u32Count, VeUInt32 u32Location)
	: VeRenderCall(DRAW), m_u32Count(u32Count), m_u32Location(u32Location)
{

}
//--------------------------------------------------------------------------
VeDrawCall::~VeDrawCall()
{

}
//--------------------------------------------------------------------------
VeDrawIndexedCall::VeDrawIndexedCall(VeUInt32 u32IndexCount,
	VeUInt32 u32StartIndex)
	: VeRenderCall(DRAW_INDEXED), m_u32IndexCount(u32IndexCount)
	, m_u32StartIndex(u32StartIndex)
{

}
//--------------------------------------------------------------------------
VeDrawIndexedCall::~VeDrawIndexedCall()
{
...
VeRenderNode::VeRenderNode(const VeChar8* pcDataName)
	: m_pkParent(NULL), m_u32Priority(VE_INFINITE)
{
	m_tContent = this;
	SetData(pcDataName);
}
