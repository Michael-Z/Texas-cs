	...
	: m_kName(pcName), m_pkZzipRoot(pkZzipRoot)
{
	...
	VeVector<VeFixedStringA> kCurrentPath;
	while (zzip_dir_read(m_pkZzipRoot, &zzipEntry))
	{
		...
		{
			VeStringA kFileName(pcFileName, u32DirNameLen);
...
{
	VE_AUTO_LOCK_MUTEX(m_spZip->GetMutex());
	VE_ASSERT(m_spZip);
...
VeBinaryIStreamPtr VeZipDir::OpenSync(const VeChar8* pcFileName) const
{
	...
	if(it)
	{
		...
		VeStringA kFullPath = m_kName + "/" + pcFileName;
