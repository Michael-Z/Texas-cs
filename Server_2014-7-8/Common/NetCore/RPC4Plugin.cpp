...
{
    void ( *registerFunctionPointer ) ( VENet::BitStream *userData, Packet *packet );
    void ( *registerBlockingFunctionPointer ) ( VENet::BitStream *userData, VENet::BitStream *returnData, Packet *packet );
...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char* uniqueID, void ( *functionPointer ) ( VENet::BitStream *userData, Packet *packet ))
{
    VEAssert(globalRegistrationIndex!=RPC4_GLOBAL_REGISTRATION_MAX_FUNCTIONS);
    unsigned int i;
    for (i=0; uniqueID[i]; i++)
    {
        VEAssert(i<=RPC4_GLOBAL_REGISTRATION_MAX_FUNCTION_NAME_LENGTH-1);
...
RPC4GlobalRegistration::RPC4GlobalRegistration(const char* uniqueID, void ( *functionPointer ) ( VENet::BitStream *userData, Packet *packet ), int callPriority)
{
    VEAssert(globalRegistrationIndex!=RPC4_GLOBAL_REGISTRATION_MAX_FUNCTIONS);
    unsigned int i;
    for (i=0; uniqueID[i]; i++)
    ...
    globalRegistrationBuffer[globalRegistrationIndex].registerFunctionPointer=functionPointer;
    globalRegistrationBuffer[globalRegistrationIndex].registerBlockingFunctionPointer=0;
    VEAssert(callPriority!=(int) 0xFFFFFFFF);
    globalRegistrationBuffer[globalRegistrationIndex].callPriority=callPriority;
    globalRegistrationIndex++;
}
RPC4GlobalRegistration::RPC4GlobalRegistration(const char* uniqueID, void ( *functionPointer ) ( VENet::BitStream *userData, VENet::BitStream *returnData, Packet *packet ))
{
    VEAssert(globalRegistrationIndex!=RPC4_GLOBAL_REGISTRATION_MAX_FUNCTIONS);
    unsigned int i;
    for (i=0; uniqueID[i]; i++)
...
RPC4::~RPC4()
{
    ...
    {
        VENet::OP_DELETE(localCallbacks[i],_FILE_AND_LINE_);
    }

    DataStructures::List<VENet::VEString> keyList;
    ...
    {
        VENet::OP_DELETE(outputList[j],_FILE_AND_LINE_);
    }
    localSlots.Clear(_FILE_AND_LINE_);
}
bool RPC4::RegisterFunction(const char* uniqueID, void ( *functionPointer ) ( VENet::BitStream *userData, Packet *packet ))
{
...
void RPC4::RegisterSlot(const char *sharedIdentifier, void ( *functionPointer ) ( VENet::BitStream *userData, Packet *packet ), int callPriority)
{
    ...
    if (idx.IsInvalid())
    {
        localSlot = VENet::OP_NEW<LocalSlot>(_FILE_AND_LINE_);
...
bool RPC4::RegisterBlockingFunction(const char* uniqueID, void ( *functionPointer ) ( VENet::BitStream *userData, VENet::BitStream *returnData, Packet *packet ))
{
...
void RPC4::RegisterLocalCallback(const char* uniqueID, MessageID messageId)
{
    ...
    {
        lc = VENet::OP_NEW<LocalCallback>(_FILE_AND_LINE_);
...
{
    void ( *f ) ( VENet::BitStream *, VENet::BitStream *,Packet * );
...
bool RPC4::UnregisterLocalCallback(const char* uniqueID, MessageID messageId)
{
    ...
    if (objectExists)
    {
        ...
        if (objectExists)
        {
            ...
            {
                VENet::OP_DELETE(lc,_FILE_AND_LINE_);
...
bool RPC4::UnregisterSlot(const char* sharedIdentifier)
{
    ...
    if (hi.IsInvalid()==false)
    {
        ...
        VENet::OP_DELETE(ls, _FILE_AND_LINE_);
...
void RPC4::CallLoopback( const char* uniqueID, VENet::BitStream * bitStream )
{
    Packet *p=0;
    ...
    if (skhi.IsInvalid()==true)
    {
        ...
        if (vePeerInterface)
            p->guid=vePeerInterface->GetGuidFromSystemAddress(UNASSIGNED_SYSTEM_ADDRESS);
#if _VENET_SUPPORT_PacketizedTCP==1 && _VENET_SUPPORT_TCPInterface==1
        else
...
void RPC4::Call( const char* uniqueID, VENet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast )
{
    VENet::BitStream out;
    out.Write((MessageID) ID_RPC_PLUGIN);
    out.Write((MessageID) ID_RPC4_CALL);
...
bool RPC4::CallBlocking( const char* uniqueID, VENet::BitStream * bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, VENet::BitStream *returnData )
{
    VENet::BitStream out;
    out.Write((MessageID) ID_RPC_PLUGIN);
    out.Write((MessageID) ID_RPC4_CALL);
    ...
    VEAssert(returnData);
    VEAssert(vePeerInterface);
    ConnectionState cs;
    cs = vePeerInterface->GetConnectionState(systemIdentifier);
    if (cs!=IS_CONNECTED)
        return false;
    ...
    while (gotBlockingReturnValue==false)
    {
        ...
        packet=vePeerInterface->Receive();

        if (packet)
        {
            if (
                (packet->data[0]==ID_CONNECTION_LOST || packet->data[0]==ID_DISCONNECTION_NOTIFICATION) &&
                ((systemIdentifier.veNetGuid!=UNASSIGNED_VENET_GUID && packet->guid==systemIdentifier.veNetGuid) ||
                 (systemIdentifier.systemAddress!=UNASSIGNED_SYSTEM_ADDRESS && packet->systemAddress==systemIdentifier.systemAddress))
            )
            {
                vePeerInterface->PushBackPacket(packet,true);
                while (packetQueue.Size())
                    vePeerInterface->PushBackPacket(packetQueue.Pop(),true);
                return false;
            }
...
void RPC4::Signal(const char *sharedIdentifier, VENet::BitStream *bitStream, PacketPriority priority, PacketReliability reliability, char orderingChannel, const AddressOrGUID systemIdentifier, bool broadcast, bool invokeLocal)
{
    VENet::BitStream out;
    out.Write((MessageID) ID_RPC_PLUGIN);
    ...
    if (invokeLocal)
    {
        ...
        Packet p;
        p.guid=vePeerInterface->GetMyGUID();
        p.systemAddress=vePeerInterface->GetMyBoundAddress(0);
        p.wasGeneratedLocally=true;
        VENet::BitStream *bsptr, bstemp;
        if (bitStream)
        {
            bitStream->ResetReadPointer();
...
void RPC4::InvokeSignal(DataStructures::HashIndex functionIndex, VENet::BitStream *serializedParameters, Packet *packet)
{
    if (functionIndex.IsInvalid())
        return;
    ...
    if (packet->data[0]==ID_RPC_PLUGIN)
    {
        ...
        if (packet->data[1]==ID_RPC4_CALL)
        {
            ...
            if (isBlocking==false)
            {
                ...
                void ( *fp ) ( VENet::BitStream *, Packet * );
            ...
            else
            {
                ...
                void ( *fp ) ( VENet::BitStream *, VENet::BitStream *, Packet * );
                fp = registeredBlockingFunctions.ItemAtIndex(skhi);
                VENet::BitStream returnData;
                bsIn.AlignReadToByteBoundary();
        ...
        {
            VENet::VEString sharedIdentifier;
            bsIn.ReadCompressed(sharedIdentifier);
            DataStructures::HashIndex functionIndex;
            functionIndex = localSlots.GetIndexOf(sharedIdentifier);
            VENet::BitStream serializedParameters;
            bsIn.AlignReadToByteBoundary();
        ...
        {
            VEAssert(packet->data[1]==ID_RPC4_RETURN);
            blockingReturnValue.Reset();
