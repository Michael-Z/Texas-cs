...
#if ONLY_MSPACES
#define internal_malloc(m, b) ve_mspace_malloc(m, b)
#define internal_free(m, mem) ve_mspace_free(m,mem);
#else /* ONLY_MSPACES */
#if MSPACES
#define internal_malloc(m, b)\
	(m == gm)? rdlmalloc(b) : ve_mspace_malloc(m, b)
#define internal_free(m, mem)\
	if (m == gm) rdlfree(mem); else ve_mspace_free(m,mem);
#else /* MSPACES */
...
void* ve_mspace_realloc(mspace msp, void* oldmem, size_t bytes) {
	if (oldmem == 0)
		return ve_mspace_malloc(msp, bytes);
#ifdef REALLOC_ZERO_BYTES_FREES
	if (bytes == 0) {
		ve_mspace_free(msp, oldmem);
		return 0;
	}
#endif /* REALLOC_ZERO_BYTES_FREES */
	else {
...
#endif // _VENET_SUPPORT_DL_MALLOC
