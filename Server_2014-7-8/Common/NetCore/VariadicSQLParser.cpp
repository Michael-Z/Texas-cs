...
void VariadicSQLParser::ExtractArguments( va_list argptr, const DataStructures::List<IndexAndType> &indices, char ***argumentBinary, int **argumentLengths )
{
    ...
    unsigned int i;
    *argumentBinary=VENet::OP_NEW_ARRAY<char *>(indices.Size(), _FILE_AND_LINE_);
    *argumentLengths=VENet::OP_NEW_ARRAY<int>(indices.Size(), _FILE_AND_LINE_);
    ...
    for (variadicArgIndex=0, i=0; i < indices.Size(); i++, variadicArgIndex++)
    {
        ...
        case 'd':
        {
            ...
            if (VENet::BitStream::IsNetworkOrder()==false) VENet::BitStream::ReverseBytesInPlace((unsigned char*) paramData[i], paramLength[i]);
        }
        break;
        case 's':
        {
...
void VariadicSQLParser::FreeArguments(const DataStructures::List<IndexAndType> &indices, char **argumentBinary, int *argumentLengths)
{
    ...
    for (i=0; i < indices.Size(); i++)
        veFree_Ex(argumentBinary[i],_FILE_AND_LINE_);
    VENet::OP_DELETE_ARRAY(argumentBinary,_FILE_AND_LINE_);
    VENet::OP_DELETE_ARRAY(argumentLengths,_FILE_AND_LINE_);
}
