...
{
    veNetGuid=packet->guid;
    systemAddress=packet->systemAddress;
}

unsigned long AddressOrGUID::ToInteger( const AddressOrGUID &aog )
{
    if (aog.veNetGuid!=UNASSIGNED_VENET_GUID)
        return VENetGUID::ToUint32(aog.veNetGuid);
...
SocketDescriptor::SocketDescriptor()
{
    ...
    port=0;
    hostAddress[0]=0;
    remotePortVENetWasStartedOn_PS3_PSP2=0;
...
{
    systemIndex=(SystemIndex)-1;
    *this=UNASSIGNED_VENET_GUID;
}
bool VENetGUID::operator==( const VENetGUID& right ) const
{
    return g==right.g;
}
bool VENetGUID::operator!=( const VENetGUID& right ) const
...
const char *VENetGUID::ToString(void) const
{
    static unsigned char strIndex=0;
...
void VENetGUID::ToString(char *dest) const
{
    if (*this==UNASSIGNED_VENET_GUID)
        strcpy(dest, "UNASSIGNED_VENet_GUID");
    else
        sprintf(dest, "%" PRINTF_64_BIT_MODIFIER "u", (long long unsigned int) g);
}
bool VENetGUID::FromString(const char *source)
{
    if (source==0)
        return false;
...
unsigned long VENetGUID::ToUint32( const VENetGUID &g )
{
