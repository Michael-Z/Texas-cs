...
STATIC_FACTORY_DEFINITIONS(VENetCommandParser,VENetCommandParser);

VENetCommandParser::VENetCommandParser()
{
    ...
    RegisterCommand(2,"AddToBanList","( const char *IP, VENet::TimeMS milliseconds=0 );");
    ...
    RegisterCommand(2,"SetTimeoutTime","( VENet::TimeMS timeMS, const SystemAddress target );");
...
void VENetCommandParser::SetRakPeerInterface(VENet::VEPeerInterface *rakPeer)
{
    peer=rakPeer;
}
bool VENetCommandParser::OnCommand(const char *command, unsigned numParameters, char **parameterList, TransportInterface *transport, const SystemAddress &systemAddress, const char *originalString)
{
    (void) originalString;
    (void) numParameters;
    ...
    {
        VENet::SocketDescriptor socketDescriptor((unsigned short)atoi(parameterList[1]), parameterList[2]);
    ...
    {
        ReturnResult(peer->Connect(parameterList[0], (unsigned short)atoi(parameterList[1]),parameterList[2],atoi(parameterList[3]))==VENet::CONNECTION_ATTEMPT_STARTED, command, transport, systemAddress);
    }
...
const char *VENetCommandParser::GetName(void) const
{
    return "VENet";
}
void VENetCommandParser::SendHelp(TransportInterface *transport, const SystemAddress &systemAddress)
{
    if (peer)
    {
        transport->Send(systemAddress, "The VENet parser provides mirror functions to RakPeer\r\n");
