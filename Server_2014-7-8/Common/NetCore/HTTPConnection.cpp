...
bool HTTPConnection::HasBadResponse(int *code, VENet::VEString *data)
{
...
VEString HTTPConnection::Read(void)
{
    if (results.IsEmpty())
        return VEString();

    VENet::VEString resultStr = results.Pop();
    const char *start_of_body = strpbrk(resultStr.C_String(), "\001\002\003%");

    if (start_of_body)
        return VENet::VEString::NonVariadic(start_of_body);
    else
        return resultStr;
}
...
void HTTPConnection::ProcessTCPPacket(Packet *packet)
{
    ...
    if (packet->systemAddress == server)
    {
        ...
        VENet::VEString incomingTemp = VENet::VEString::NonVariadic((const char*) packet->data);
        incomingTemp.URLDecode();
        incomingData += incomingTemp;

        VEAssert(strlen((char *)packet->data) == packet->length);
