...
class VE_POWER_API VeBinaryOStream : virtual public VeSharedRefObject
{
public:
...
class VE_POWER_API VeBinaryIStream : virtual public VeSharedRefObject
{
public:
	VE_INLINE VeBinaryIStream();
	...
	void GetStringAligned4(VeStringA& kStr);	
...
VE_FORCE_INLINE VeBinaryOStream& operator << (VeBinaryOStream& kOut, const VeStringA& kStr)
{
...
{
    VeUInt32 u32Size = kData.Size();
	kOut.WritePackedInt((VeInt32)u32Size);
...
VE_FORCE_INLINE VeBinaryIStream& operator >> (VeBinaryIStream& kIn, VeStringA& kStr)
{
...
{
	VeUInt32 u32Size = kIn.ReadPackedInt();
	T tVal;
	kData.Clear();
	kData.Reserve(u32Size);
	for(VeUInt32 i(0); i < u32Size; ++i)
	{
		kIn >> tVal;
		kData.PushBack(tVal);
