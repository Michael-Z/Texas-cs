...
#	if defined(VE_PLATFORM_WIN)
	...
	struct Output : public VeMemObject
	{
		typedef VePair<VeUInt32,VePair<VeUInt32,VeUInt32>> Mode;
		typedef VeHashMap<Mode,VeUInt32,Output> ModeMap;
		typedef VeVector<DEVMODE> ModeList;
		
		static VeUInt32 KeyToHashCode(const Mode& kKey, VeUInt32 u32TableSize);

		DISPLAY_DEVICE m_kDevice;
		ModeMap m_kModeMap;
		ModeList m_kModeList;
		DEVMODE m_kDefault;
	};
	...
	VeStringMap<VeUInt32> m_kAdapterMap;
