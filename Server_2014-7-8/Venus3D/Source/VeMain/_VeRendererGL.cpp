...
void VeRendererGL::Init()
{
	...
	VeDWord dwDisp(0);
	DISPLAY_DEVICE kDisplayDevice;
	VeZeroMemory(&kDisplayDevice, sizeof(DISPLAY_DEVICE));
	kDisplayDevice.cb = sizeof(DISPLAY_DEVICE);

	while(EnumDisplayDevices(NULL, dwDisp++, &kDisplayDevice, 0))
	{
		VeUInt32* pu32AdapterIndex = m_kAdapterMap.Find(kDisplayDevice.DeviceString);
		if(!pu32AdapterIndex)
		{
			pu32AdapterIndex = &(m_kAdapterMap[kDisplayDevice.DeviceString]);
			*pu32AdapterIndex = m_kAdapterList.Size();
			m_kAdapterList.Resize(m_kAdapterList.Size() + 1);
		}
		Adapter& kAdapter = m_kAdapterList[*pu32AdapterIndex];
		kAdapter.m_kOutputList.Resize(kAdapter.m_kOutputList.Size() + 1);
		Output& kOutput = kAdapter.m_kOutputList.Back();
		kOutput.m_kDevice = kDisplayDevice;

		VeDWord dwMode = 0;
		DEVMODE kDevMode;
		while(EnumDisplaySettings(NULL, dwMode++, &kDevMode))
		{
			if(kDevMode.dmBitsPerPel != 32)
				continue;
			Output::Mode kKey(kDevMode.dmDisplayFrequency, VePair<VeUInt32,VeUInt32>(kDevMode.dmPelsWidth, kDevMode.dmPelsHeight));
			VeUInt32* pu32OutputIndex = kOutput.m_kModeMap.Find(kKey);
			if(!pu32OutputIndex)
			{
				pu32OutputIndex = &(kOutput.m_kModeMap[kKey]);
				*pu32OutputIndex = kOutput.m_kModeList.Size();
				kOutput.m_kModeList.Resize(kOutput.m_kModeList.Size() + 1);
				kOutput.m_kModeList[*pu32OutputIndex] = kDevMode;
				if((*pu32OutputIndex) == 0)
				{
					kOutput.m_kDefault = kDevMode;
				}
				else
				{
					if(kDevMode.dmDisplayFrequency > kOutput.m_kDefault.dmDisplayFrequency
						|| kDevMode.dmPelsWidth > kOutput.m_kDefault.dmPelsWidth
						|| kDevMode.dmPelsHeight > kOutput.m_kDefault.dmPelsHeight)
					{
						kOutput.m_kDefault = kDevMode;
					}
				}
	...
	VeLoadOpenGL();
	wglSwapIntervalEXT(g_pStartConfig->IsSync() ? 1 : 0);
	UpdateBufferSize();
...
void VeRendererGL::Term()
{
	...
	m_hRenderContext = NULL;
	ReleaseDC(g_hWindow, m_hDeviceContext);
	m_hDeviceContext = NULL;
