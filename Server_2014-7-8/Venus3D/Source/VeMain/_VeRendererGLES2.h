...
#	endif
	...
	virtual bool CreateVertexShader(const VeChar8* pcName, VeBinaryIStream& kStream, VeROVertexShaderPtr* pspShader);

	virtual bool CreatePixelShader(const VeChar8* pcName, VeBinaryIStream& kStream, VeROPixelShaderPtr* pspShader);

	virtual bool CreateBlendState(const VeChar8* pcName, VeBinaryIStream& kStream, VeROBlendStatePtr* pspBlendState);

	virtual bool CreateDepthStencilState(const VeChar8* pcName, VeBinaryIStream& kStream, VeRODepthStencilStatePtr* pspDepthStencilState);

	virtual bool CreateInputLayout(const VeChar8* pcName, VeBinaryIStream& kStream, VeROInputLayoutPtr* pspLayout);

	virtual bool CreateRasterizerState(const VeChar8* pcName, VeBinaryIStream& kStream, VeRORasterizeStatePtr* pspRasterizeState);

	virtual bool CreateBuffer(const VeChar8* pcName, VeBinaryIStream& kStream, VeROBufferPtr* pspBuffer);

	virtual bool CreateSamplerState(const VeChar8* pcName, VeBinaryIStream& kStream, VeROSamplerStatePtr* pspSamplerState);

	virtual bool CreateTexture2D(const VeChar8* pcName, VeBinaryIStream& kStream, VeROTexture2DPtr* pspTexture);
...
protected:
	...
	VeVector4D m_akVSCBufferCache[256];
	VeUInt32 m_u32VSCBufferCachePointer;
	...
	struct Description
	{
		...
		VeVector<VeFixedStringA> m_kExtensionArray;
	};
	...
	VeVector<VeUInt32> m_kProgramArray;
	...
	void ClearColor(const VeColorA& kColor);
	...
	VeInt32 m_i32VSCBufferLoc;
	VeInt32 m_i32PSCBufferLoc;
	VE_BOOL m_bVSCBufferNeedUpdate;
	VE_BOOL m_bPSCBufferNeedUpdate;
	bool m_bEnableVSTexture;
	bool m_bEnablePSTexture;
	bool m_abVertexAttribArrayState[VE_GLES2_IA_VERTEX_ATTRIB_ARRAY_NUM];
